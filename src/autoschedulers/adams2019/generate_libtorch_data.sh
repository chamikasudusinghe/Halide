...
BASELOC="/home/bhavya/cosmos/life/UIUC/academics/research/halide/halide-data"
...
AUTOSCHED_BIN=${HALIDE_INSTALL_ROOT}/bin
AUTOSCHED_LIB=${HALIDE_INSTALL_ROOT}/lib
HALIDE_DISTRIB_PATH=${HALIDE_INSTALL_ROOT}
SAMPLES="/home/bhavya/cosmos/life/UIUC/academics/research/halide/halide-training"
START_WEIGHTS_FILE=${AUTOSCHED_TOOLS}/baseline_libtorch.pt
export HL_WEIGHTS_DIR=${START_WEIGHTS_FILE}
...
HL_TARGET=`${AUTOSCHED_BIN}/adams2019_get_host_target avx512 avx512_knl avx512_skylake avx512_cannonlake`
fi
...
make_featurization() {
	...
	${TIMEOUT_CMD} -k ${COMPILATION_TIMEOUT} ${COMPILATION_TIMEOUT} \
		...
		-p ${AUTOSCHED_LIB}/libautoschedule_adams2019.${PLUGIN_EXT} \
	...
	c++ \
		...
		${HALIDE_DISTRIB_PATH}/share/tools/RunGenMain.cpp \
		${D}/*.registration.cpp \
		${D}/*.a \
		-o ${D}/bench \
...
benchmark_sample() {
	...
	S=$2
	FNAME=$4
	${AUTOSCHED_BIN}/adams2019_featurization_to_sample ${D}/${FNAME}.featurization $R $P $S ${D}/${FNAME}.sample || echo "featurization_to_sample failed for ${D} (probably because benchmarking failed)"
}
...
for ((BATCH_ID=$((FIRST+1));BATCH_ID<$((FIRST+1+NUM_BATCHES));BATCH_ID++)); do
	...
	for ((EXTRA_ARGS_IDX=0;EXTRA_ARGS_IDX<${#GENERATOR_ARGS_SETS_ARRAY[@]};EXTRA_ARGS_IDX++)); do
		...
		for ((SAMPLE_ID=0;SAMPLE_ID<${BATCH_SIZE};SAMPLE_ID++)); do
			echo "	FEATURIZING BATCH ${BATCH_ID} SAMPLE ${SAMPLE_ID}"
			while [[ 1 ]]; do
				RUNNING=$(jobs -r | wc -l)
			...
			## to generate multiple schedules per pipeline
			BEAMSIZE=${SAMPLE_ID}
			make_featurization "${DIR}/${SAMPLE_ID}" $S $FNAME $BEAMSIZE $PIPELINE_NUM "$EXTRA_GENERATOR_ARGS" &
			echo -n .
		done
		wait
		echo  done.
		...
		for ((SAMPLE_ID=0;SAMPLE_ID<${BATCH_SIZE};SAMPLE_ID++)); do
			echo "	BENCHMARKING BATCH ${BATCH_ID} SAMPLE ${SAMPLE_ID}"
			S=$(printf "%04d%04d" $BATCH_ID $SAMPLE_ID)
			FNAME=$(printf "%s_batch_%04d_sample_%04d" ${PIPELINE} $BATCH_ID $SAMPLE_ID)
			benchmark_sample "${DIR}/${SAMPLE_ID}" $S $EXTRA_ARGS_IDX $FNAME $PIPELINE_NUM
		done
